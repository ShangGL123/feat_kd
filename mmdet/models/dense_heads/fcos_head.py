...
class FCOSHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if self.identity_pos != -1:
            self.cls_identities = nn.ModuleList([IdentityModule() for _ in regress_ranges])
            self.reg_identities = nn.ModuleList([IdentityModule() for _ in regress_ranges])
        else:
            self.cls_identities = [None for _ in regress_ranges]
            self.reg_identities = [None for _ in regress_ranges]
    ...
    def forward(self, feats):
        ...
        """
        return multi_apply(self.forward_single, feats, self.scales, self.strides, self.cls_identities, self.reg_identities)

    def forward_single(self, x, scale, stride, cls_identity, reg_identity):
